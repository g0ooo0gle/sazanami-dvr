...
`setup`はサービスごとに短時間の確認を行い、PATで確認できない場合は必要に応じて放送内のサービス一覧を自動確認します。
録画やライブ視聴が動いている場合は、空きチューナーが足りず失敗することがあります。確認できないサービスがある場合も、
既存の`channels.json`は上書きしません。同じ内容なら`unchanged`、異なる内容なら失敗として残します。
