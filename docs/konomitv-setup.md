...
放送中の番組へ新しく録画操作を行った場合は、終了まで60秒以上残っていれば現在時刻から録画します。
冒頭の過ぎた区間は保存しません。以前から残っている予約は、予定開始から5分を超えると安全のため開始しません。

録画処理は、予約の前後余白を反映した時刻に動きます。既定では番組開始の5秒前から番組終了の2秒後までです。

## ワンセグ別出力を確認する

KonomiTV v0.14.1の同梱画面には、この設定項目は表示されません。予約APIでワンセグ別出力を有効にした要求を送ると、通常の`.ts`に加えて`.oneseg.ts`を保存します。ワンセグ用録画先を空欄にした場合は、本編と同じ録画先とファイル名規則を使います。別の録画先を使う場合も、`recording-root`からの相対フォルダーを一件だけ指定してください。

Sazanami DVRは、同じ放送波に部分受信サービスが一件だけある場合に予約を受け付けます。候補がない、または複数ある場合は別サービスを推測せず、予約の追加・変更を失敗させます。ワンセグだけが録画に失敗しても、本編が正常なら本編の`.ts`は残ります。現在は合成データを使う自動テストまで確認済みで、実MirakurunとKonomiTV画面を組み合わせた確認は未実施です。
...
| `.oneseg.ts`が作られない | ワンセグ別出力が有効か、同じ放送波の部分受信サービスを一件だけ取得できているか、ワンセグ用録画先が相対フォルダーか |
