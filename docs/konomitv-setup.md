...
`recording serve`は起動直後と既定1時間ごとに番組表を更新します。更新に失敗した場合は前回の番組表を維持し、録画処理を止めません。

KonomiTVから番組追従ありで作った予約は、録画開始前に限り、新しい開始・終了時刻へ追従します。同じMirakurun内の同じ番組で、観測間隔と時刻差が固定上限内の場合だけ更新します。判断できない場合は予約時の時刻を維持します。録画開始後の延長や短縮には対応していません。
