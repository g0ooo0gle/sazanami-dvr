...
確認対象はKonomiTV v0.14.1です。2026年8月5日に、画面の番組表から予約し、5分間録画したファイルを
録画済み一覧から再生するところまで一件確認しました。長時間運転や幅広い環境での動作は未確認です。
...
6. 番組終了後、録画保存先に`.ts`の完成ファイルが一件あることを確認する
7. KonomiTVの「ビデオをみる」に録画済み番組が表示されることを確認する

実験では、終了まで60秒以上残っている番組を選んでください。開始から5分を超えた番組や、残り60秒未満の
番組は安全のため録画しません。

現在は予約時刻になってからMirakurunのストリームを開きます。確認した一件では録画開始が予定より約3秒遅れ、
KonomiTV上で「一部のみ録画」と表示されました。ファイル作成と再生は完了していますが、この開始遅延は今後
改善します。
