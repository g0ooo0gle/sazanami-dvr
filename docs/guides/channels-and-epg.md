...
- 対応する放送サービスのストリームを短時間読み、PATからTSIDを確認する。PATで確認できない場合は、同じチャンネルの放送内サービス一覧（SDT actual）から自動確認する
...
サービス確認ではサービスごとに一時的なストリーム接続を使います。録画やライブ視聴が動いていると空きチューナーが
足りないことがあるため、停止してから再実行してください。PATで確認できないサービスは、利用可能なチャンネル情報を
使って同じチャンネルの放送内サービス一覧を自動確認します。所属を確認できない場合は`channels.json`を保存せず失敗します。
録画TSやPAT・SDTの内容は保存しません。
