...
`setup`はDBの初期化または状態確認、起動前の復旧と古い番組表の自動整理、番組表同期、サービスの自動確認、
`data/sazanami/channels.json`の生成をまとめて行います。PATで確認できないサービスは、必要に応じて放送内の
サービス一覧から自動確認します。確認できないサービスがある場合は設定を保存せず終了します。
成功時は`result=completed`と表示されます。
