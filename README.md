...
現在のバージョンは **v0.0.25（ベータ版）** です。基本的な録画の流れは動きますが、対応範囲はまだ限定的です。
...
- 自動予約で実行できる検索条件と録画設定は初版の安全な組み合わせに限定。対応形式の相対録画先、ファイル名、字幕・データ放送、録画後スクリプトは使えるが、未対応の条件は保存するだけで予約を作らない
