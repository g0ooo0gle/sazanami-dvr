...
現在のバージョンは **v0.0.23（ベータ版）** です。基本的な録画の流れは動きますが、対応範囲はまだ限定的です。
...
v0.0.12では、Mirakurunから番組詳細、ジャンル、映像、音声を取得して番組表へ保存します。KonomiTVの番組表示と検索へ同じ情報を返し、ジャンル検索、ジャンル除外、映像・音声の絞り込み、表記の違いや小さな入力違いを許すあいまい検索を追加しました。v0.0.13のジャンル条件には誤りがあるため、使用しないでください。v0.0.14ではKonomiTVが実際に送る通信上のバイト順へ戻し、その並びを固定値で検査しています。

v0.0.15では、KonomiTVから録画中の予約を取り消すと、その一件だけを停止します。188バイト以上を保存できていれば通常の`.ts`名で公開し、利用者が途中で止めた部分録画だとDBへ残します。通常終了とは区別したまま、CtrlCmdの録画一覧、録画履歴API、HTTP Range再生から読み出せます。

v0.0.16では、KonomiTVの基本録画設定に対応しました。予約の有効・無効、優先度1～5、番組追従、既定余白、前後それぞれ-3,600～3,600秒の個別余白を、追加・一覧・変更で同じ値のまま扱います。既定では番組開始5秒前から番組終了2秒後まで録画します。無効予約は録画せず一覧に残り、開始前なら再び有効にできます。自動予約から作る予約にも同じ設定を引き継ぎます。

v0.0.17では、CtrlCmdを追加設定なしで同じLANから使えるようにしました。`recording serve`と`ctrlcmd serve`は、既定でIPv4の全interfaceに待ち受けます。認証はないためインターネットへ直接公開しないでください。同じPCだけに絞る場合は`--listen 127.0.0.1:4510`を指定します。録画再生HTTPとWebUIは従来どおり同じPCからだけ接続できる既定です。

v0.0.18では、KonomiTVとKomorebiの局ロゴ取得に対応しました。完成済み番組表と一致する放送局だけをMirakurunへ問い合わせます。画像はSazanamiへ保存せず、ロゴがない場合や通信に失敗した場合は、番組表や録画を止めずに各クライアントの標準画像へ戻します。実験環境では、KonomiTVとKomorebi相当の取得結果が一致し、KonomiTVのロゴAPIも正常に応答しました。

v0.0.19では、KonomiTVの予約画面で指定した録画先フォルダーとファイル名テンプレートを、実際の録画ファイルへ反映します。録画先は`--recording-root`からの相対フォルダーだけを受け、絶対パスや親フォルダーへの参照は拒否します。番組名、放送局名、開始・終了日時、放送時間、放送ID、予約番号の代表的なEDCBマクロをGoだけで展開します。単発予約と自動予約で同じ設定を使います。

v0.0.20では、利用者の`umask`が0002の場合もSQLiteのWALとSHMを0600へ揃えます。DB更新前のバックアップを、一般的なUbuntu環境で厳しい権限検査を保ったまま復元できるようにしました。DB形式と保存内容はv0.0.19から変わりません。

v0.0.21では、KonomiTVの字幕・データ放送設定を予約へ保存し、実際の録画TSへ反映します。既定は字幕あり、データ放送なしです。どちらかを除く録画では、PATとPMTを小さい上限内で読み、PMTと対象PIDを整合させたまま保存します。TS形式を安全に確認できない場合は、その録画を`STREAM_FORMAT_INVALID`で失敗させます。

v0.0.22では、KonomiTVの「デフォルト」「何もしない」と録画後スクリプトを予約へ保存します。スクリプトは`<data-root>/post-recording-scripts`内の実行可能ファイルだけを、録画とDBの確定後に一度実行します。shellや自動再試行は使わず、5分で終了します。スクリプトが失敗しても完成した録画は残ります。スタンバイ、休止、再起動、シャットダウンは未対応です。

v0.0.23では、保存先とファイル名を指定した予約をKonomiTVから変更できない問題を修正しました。KonomiTVが予約一覧から送り返す予定ファイル名は互換用の値として読み取り、保存先には使いません。保存先は、従来どおり検証済みの録画フォルダー設定だけから決めます。
