...
現在のバージョンは **v0.0.4（ベータ版）** です。基本的な録画の流れは動きますが、対応範囲はまだ限定的です。
...
MirakurunとKonomiTV v0.14.1を使い、番組表の表示、予約、約26分の録画、録画中の番組表更新、再起動後の状態確認、録画済み一覧からの再生開始を確認しています。通信切断後の再接続は自動テストまでで、実環境では未確認です。
