...
現在のバージョンは **v0.0.21（ベータ版）** です。基本的な録画の流れは動きますが、対応範囲はまだ限定的です。
...
- 予約変更は録画開始前の有効状態、優先度、番組追従、前後余白、相対録画先、対応済みファイル名テンプレート、字幕・データ放送の選択に限定
- 録画中の停止で再生可能にするのは、利用者が明示的に止め、188バイト以上を保存できた一件だけ。通信障害や容量不足による部分録画は公開しない
- 自動予約で実行できる検索条件と録画設定は初版の安全な組み合わせに限定。対応形式の相対録画先、ファイル名、字幕・データ放送の選択は使えるが、未対応の条件は保存するだけで予約を作らない
