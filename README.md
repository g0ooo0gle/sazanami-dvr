...
現在のバージョンは **v0.0.10（ベータ版）** です。基本的な録画の流れは動きますが、対応範囲はまだ限定的です。
...
v0.0.10では、KonomiTVとKomorebiが使うライブ視聴の開始・中継・終了を追加しました。通信形式と資源解放は自動テスト済みです。対象クライアントの画面を使う実環境確認は、公開配布物で行った結果を[互換実装表](docs/compatibility.md)へ記載します。
