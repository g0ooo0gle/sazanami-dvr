...
現在のバージョンは **v0.0.11（ベータ版）** です。基本的な録画の流れは動きますが、対応範囲はまだ限定的です。
...
- KonomiTVから、保存済みの番組表をキーワード、放送局、日時、番組長、無料・有料で検索する
...
- 番組検索のジャンル、映像・音声、あいまい検索は未対応。キーワード、除外語、正規表現、番組名限定、放送局、日時、番組長、無料・有料は利用可能
