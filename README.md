...
現在のバージョンは **v0.0.1（ベータ版）** です。基本的な録画の流れは動きますが、対応範囲はまだ限定的です。
...
MirakurunとKonomiTV v0.14.1を使い、番組表の表示、予約、5分間の録画、再起動後の状態確認、録画済み一覧からの再生を一件確認しています。長時間運転や幅広い環境での互換性は未確認です。
