...
func (stream *e2eStream) OpenStream(context.Context, providerstream.Request) (providerstream.Lease, error) {
	stream.opens++
	return &e2eLease{stream: stream}, nil
}

type e2eLease struct{ stream *e2eStream }
...
func TestReservationToFinalFileSurvivesRestart(t *testing.T) {
	...
	stream := &e2eStream{clock: clock, end: reservation.Program.Start.Add(reservation.Program.Duration)}
	...
	if err != nil || result.State != core.AttemptSucceeded || stream.opens != 1 {
	...
	if err != nil || len(items) != 1 || items[0].State != core.AttemptSucceeded || items[0].ByteCount != 188 ||
		items[0].Availability != core.AvailabilityFinal || items[0].Recovered || stream.opens != 1 {
